...
void jacobian(mat j, int *f, vec x1,OtherData) //pointer of function and column of vectors

 x=x1(:); %Ensures a column vector
 ...
 xPlus_h=bsxfun(@times, ones(1,l), x)+diag(x_h);
    for i=1:l
        fx1=(feval(f,xPlus_h(:,i),OtherData)-feval(f,x,OtherData));
        fx=fx1(:); %Ensures a column vector
